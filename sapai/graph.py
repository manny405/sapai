...
def prep_pet_str(pstr):
    ...
    if len(temp_pstr_list) == 2:
        ...
        temp_pstr_list[1] = "{},{}".format(temp_stats[0:replace_index], 
                                           temp_stats[replace_index+1:])
...
def prep_effect(effect_list):
    ...
    if len(target_str) == 0:
        target_str = " "
    effect_columns = ["Effect", "Team", "  Pet Index  ", "  Activating Pet  ", "Targets"]
    return ([effect_name, str(team_idx), str(pet_idx), pet_str, target_str],
            effect_columns)
    ...
    for turn_name,phase_dict in f.battle_history.items():
        if turn_name == "init":
            ...
            temp_table = html_table(
                ...
                cell_font_attr=[],
                cell_border=0,
                column_align=["RIGHT", "CENTER", "CENTER", "CENTER", "CENTER", "CENTER"],
                cell_bg_colors=[[]])
            g.node(str(node_idx), 
                style="rounded,invisible",
                shape="box",
                label=temp_table)
